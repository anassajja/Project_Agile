...
if (!empty($data['first_name']) || !empty($data['last_name'])) { // Check if the first name and last name are not empty
    $stmt = $pdo->prepare('INSERT INTO students (first_name, last_name, email, department_id) VALUES (:first_name, :last_name, :email, :department_id)'); // Prepare the query to insert the department
    ...
    if ($stmt->execute([ // Execute the query with the name and description
        ...
        echo json_encode(['error' => 'Failed to add department.']); // Return an error message if the student is not added successfully
    }
} else {
